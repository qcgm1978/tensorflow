...
class ML {
    ...
    constructor(formula, toLearn, def) {
        ...
        this.def = def;
        const arr = this.getTolearnNum(formula, toLearn);
        this.iniRandomArr = this.iniRandom(arr.length);
        this.iniCoeff = this.getArray(this.iniRandomArr);
        this.iniState()
        this.iniPredict();
        this.plot()
    }
    static get NUM_POINTS() {
        return 100;
    }
    iniState() {
    ...
    getTolearnNum(formula, toLearn) {
        ...
        const reg = new RegExp(toLearn, 'g'), arr = [];
        for (let i = 0; i < len; i++) {
        ...
        */
        ...
        async function train(xs, ys, numIterations) {
            for (let iter = 0; iter < numIterations; iter++) {
                ...
                //     d: d.dataSync()[0],
                // };
                this.learning = this.generateData(ML.private.NUM_POINTS, this.iniCoeff);
                this.plot();
        ...
        function predict(x) {
            ...
            return tf.tidy(() => {
                return that.iniRandomArr[0].mul(x.pow(tf.scalar(3, 'int32')))
                    .add(that.iniRandomArr[1].mul(x.square()))
                    .add(that.iniRandomArr[2].mul(x))
                    .add(that.iniRandomArr[3]);
            });
        }
...
const ml = new ML('coef*x^3+coef*x^2+coef*x+coef', 'coef', [-0.8, -0.2, 0.9, 0.5]);
